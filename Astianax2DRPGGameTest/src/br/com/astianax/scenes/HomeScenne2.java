 ...
 * 25 de jun. de 2025
 */
public class HomeScenne2 extends AbstractScenne
{
	
	/** The rect. */
	private Rectangle rect = new Rectangle(11*GameConstants.TILESIZE, 19*GameConstants.TILESIZE+32, 128, 32);
	
	/** The porao 1. */
	private Rectangle porao1 = new Rectangle(2*GameConstants.TILESIZE, 16*GameConstants.TILESIZE, 64, 64);
	
	/** The porao 2. */
	private Rectangle porao2 = new Rectangle(16*GameConstants.TILESIZE, 16*GameConstants.TILESIZE, 64, 64);
	
	/**
	 * Instantiates a new home scenne 2.
	 *
	 ...
	 */
	public HomeScenne2(AbstractPlayer player, Game game, TileManager tmanager, AbstractGameObject[] entities,
...
//		player.geteContainer().worldY/GameConstants.TILESIZE);	
		...
		if(rect.intersects(xPos,yPos,player.solidArea.getWidth(),player.solidArea.getHeight()))
		{
			...
			TileCollision tCollision = new TileCollision();
			
			IslandScenne play = new IslandScenne(player, game, tmanager, entities, tCollision);
		
			play.start();
			player.xPos = 26*GameConstants.TILESIZE-16;
			player.yPos = 12*GameConstants.TILESIZE-32;
		...
		else if(porao1.intersects(xPos,yPos,player.solidArea.getWidth(),player.solidArea.getHeight()))
		{
			...
			HomePoraoScenne1 home = new HomePoraoScenne1( player,game, tm,entities,new TileCollision());	
			home.homeNumber = 1;
			home.start();
		...
		else if(porao2.intersects(xPos,yPos,player.solidArea.getWidth(),player.solidArea.getHeight()))
		{
			...
			HomePoraoScenne2 home = new HomePoraoScenne2( player,game, tm,entities,new TileCollision());	
			home.homeNumber = 1;
			home.start();
